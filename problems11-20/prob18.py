...
def sumRow(rowData, rowNum):
    ...
    # recursive case
    else: return sumRow(rowData, rowNum-1)
 
# read in the data
rows = []
with open('c:/scripts/newdir/euler/prob18tri.txt') as f:
    for line in f:
        rows.append([int(i) for i in line.rstrip('\n').split(" ")])
 

result = sumRow(rows, len(rows)-2) # start at second to last row
print result